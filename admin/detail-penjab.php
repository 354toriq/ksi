	...
	<div class="panel-body">
			...
			<div class="form-group"><label for="editor1">Telp.</label><input type="number" class="form-control" name="telp" required  value="<?=$penjab->kontak ?>"></div>
	...
	<a href="?lihat-penjab" class="btn btn-success btn-warning btn-lg">Kembali</a>
	</div>
	</form>
    </div>
	...
	if(isset($_POST['ubah']))
	{
		...
		$isi			=	"UPDATE `penjab` SET `nama` = '$nm',`jnsklmn` = '$jnsklmn', `tmp_lhr` = '$tmp_lhr', `tgl_lhr` = '$tgl_lhr', `pdkn` = '$pdkn', `jenjang` = '$jenj', `kontak` = '$telp', `prov` = '$prov', `kota` = '$kota', `alamat` = '$alamat', `email` = '$email', `password` = '$password' , `foto` = '$foto' WHERE `penjab`.`id_penjab` = '$idpenjab'";
		//echo $isi;
		if($foto		!=	$penjab->foto)
		{
