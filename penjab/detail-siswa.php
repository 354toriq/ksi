	...
	<div class="panel-body">
			...
			<div class="form-group"><label for="editor1">Telp.</label><input type="number" class="form-control" name="telp" required value="<?=$sis->telp?>"></div>
	...
	if(isset($_POST['ubah']))
	{
        ...
        $errors[] = "File Gambar harus kurang dari 700 KB ukuran sekarang Ijazah -> $sizeijazah KB , SKHUN -> $sizeskhun KB , Foto -> $sizefoto KB";
		}
		if(($sizeijazah >= $maxsize) ) {
        $errors[] = "File Gambar Ijazah harus kurang dari 700 KB ukuran sekarang $sizeijazah";
		}
		if(($sizeskhun >= $maxsize) ) {
        $errors[] = "File Gambar SKHUN harus kurang dari 700 KB, ukuran sekarang $sizeskhun";
		}
		if(($sizefoto >= $maxsize) ) {
        $errors[] = "File Gambar Foto harus kurang dari 700 KB ukuran sekarang $sizefoto";
		}
		...
		$isi		=	"UPDATE `siswa` SET  `id_paket` = '$idpaket',`nama` = '$nm', `jnsklmn` = '$jnsklmn', `tmp_lhr` = '$tmp_lhr', `tgl_lhr` = '$tgl_lhr', `agama` = '$agama', `nama_ortu` = '$ortu', `pdkn` = '$pdkn', `jenjang` = '$jenj', `thn_lulus` = '$thn_lulus', `no_ijasah` = '$no_ijazah', `no_skhun` = '$no_skhun', `foto` = '$foto', `foto_ijasah` = '$img_ijazah', `foto_skhun` = '$img_skhun', `email` = '$email', `prov` = '$prov', `kota` = '$kota', `telp` = '$telp', `alamat` = '$alamat', `password` = '$password' WHERE `id_siswa` = '$idsiswa'";
		//echo $isi;
		...
		<meta http-equiv="refresh" content="2;URL='data-siswa.php?detail=<?=$idsiswa?>'">
